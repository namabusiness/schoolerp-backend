...
export class ExaminationsService {
  ...
  public clearExamsCache(schoolId?: string) {
    this.fastCache.delByPrefix('exams:');
  }

  async getExams(schoolId: string) {
    const resolvedId = await this.resolveSchoolId(schoolId);
    return this.fastCache.getOrSet(`exams:list:${resolvedId}`, async () => {
  ...
  async createExam(schoolId: string, data: any) {
    ...
    this.clearExamsCache(resolvedId);
    return res;
  }

  async addExamSchedule(schoolId: string, examId: string, data: any) {
    const resolvedId = await this.resolveSchoolId(schoolId);
    const res = await this.prisma.examSchedule.create({
      data: {
        schoolId: resolvedId,
        examId,
  ...
  async getReportCard(schoolId: string, studentId: string, examId: string) {
    ...
    const cacheKey = `exams:rc:${resolvedSchoolId}:${studentId}:${examId || 'default'}`;
  ...
  async getQuestionPapers(schoolId: string, params: { classId?: string; subjectId?: string; teacherId?: string; category?: string }) {
    const resolvedId = await this.resolveSchoolId(schoolId);
    const cacheKey = `exams:qp:${resolvedId}:${params.classId || ''}:${params.subjectId || ''}:${params.teacherId || ''}:${params.category || ''}`;
    return this.fastCache.getOrSet(cacheKey, async () => {
      const where: any = {
        OR: [{ schoolId: resolvedId }, { schoolId }],
      };
  ...
  async createQuestionPaper(schoolId: string, data: any) {
    const resolvedId = await this.resolveSchoolId(schoolId);
    const res = await this.prisma.questionPaper.create({
      data: {
        schoolId: resolvedId,
        classId: data.classId,
        subjectId: data.subjectId,
  ...
  async deleteQuestionPaper(schoolId: string, id: string) {
    const res = await this.prisma.questionPaper.delete({
      where: { id },
    });
    this.clearExamsCache(schoolId);
    return res;
  }
